// Keywords client — CRUD with premium dark theme matching landing page
...
export function KeywordsClient({ projectId, keywords: initial, limit, plan }: KeywordsClientProps) {
  ...
  const isAtLimit = limit !== Infinity && keywords.length >= limit
  ...
  async function handleAdd() {
    ...
    const { data, error } = await supabase
      .from('keywords')
      .insert({ project_id: projectId, keyword: kw })
      .select()
      .single()
    setSaving(false)
    if (!error && data) {
  ...
  async function handleDelete() {
    ...
    const { error } = await supabase.from('keywords').delete().eq('id', deleteId)
    setDeletingId(null)
    if (!error) {
    ...
    <div className="space-y-6">
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          ...
          <p className="mt-1 text-[0.88rem] text-[#a1a1aa]">
            Manage the intent keywords SubHuntr monitors for you
          </p>
        </div>
      ...
      {isAtLimit && (
        <UpgradeNudge
          ...
          description={`Your ${plan} plan allows ${limit} keywords. Upgrade to add more.`}
          compact
        />
      )}
      ...
      {keywords.length === 0 ? (
        ...
        >
          ...
          <p className="mb-5 text-[0.82rem] text-[#a1a1aa]">
            Add keywords to start monitoring Reddit for leads
          </p>
          <button
        ...
        <div className="flex flex-col gap-1.5">
          {keywords.map((kw, i) => (
            ...
            >
              {/* Keyword name */}
              <span className="min-w-0 flex-1 truncate text-[0.88rem] font-semibold text-[#fafafa]">
                {kw.keyword}
              </span>
              ...
              <button
                ...
                style={{
                  background: kw.is_active ? '#1D9E75' : 'rgba(255,255,255,0.08)',
              ...
              >
                <span
                  ...
                  style={{
                    transform: kw.is_active ? 'translateX(16px)' : 'translateX(0)',
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="mb-5 text-[0.78rem] text-[#52525b]">
              SubHuntr will monitor Reddit for posts matching this keyword.
            </p>
            <input
              autoFocus
              value={newKeyword}
              ...
              placeholder="e.g. best CRM, alternative to Mailchimp"
              className="mb-5 h-11 w-full rounded-[10px] border border-[rgba(255,255,255,0.06)] bg-[#09090b] px-4 text-[0.88rem] text-[#fafafa] placeholder:text-[#52525b] focus:border-[#1D9E75] focus:outline-none"
              style={{ transition: 'border-color 0.2s' }}
            />
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={() => { setAddOpen(false); setNewKeyword('') }}
      ...
      {deleteId && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          >
            ...
            <p className="mb-5 text-[0.82rem] text-[#a1a1aa]">
              This keyword will stop being monitored. This action cannot be undone.
            </p>
            <div className="flex items-center justify-end gap-3">
              <button
                onClick={() => setDeleteId(null)}
